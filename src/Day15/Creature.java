...
public class Creature {
    ...
    private ArrayList<Point> getReachablePointsInRange(State state, Point location) {
        ArrayList<Point> reachable = getReachable(state, location);
        ArrayList<Point> pointsInRange = getPointsInRange(state);
        pointsInRange.retainAll(reachable);
        return pointsInRange;
    }

    private ArrayList<Point> getReachable(State state, Point location) {
    ...
    public void attack (State state) {
        ...
        if (neighbouringCreatures.size() > 0) {
            ...
            if (lowestScoringCreatureReadingOrder.getScore() < 0) {
                ...
                state.setSquare(lowestScoringCreatureReadingOrder.getLocation().x, lowestScoringCreatureReadingOrder.getLocation().y, '.');
            }
        }
    }

    public TreeMap<Integer, ArrayList<ArrayList<Point>>> getShortestPath (ArrayList<Point> pointsInRange, State state) {
        TreeMap<Integer, ArrayList<ArrayList<Point>>> solution = new TreeMap<>();
        TreeMap<Integer, ArrayList<ArrayList<Point>>> fringe = new TreeMap<>();
        ...
        fringe.put(0, initialArrayListOfPaths);
        while (fringe.size() > 0) {
            ...
            if (nextToPointInRange) {
                if (solution.keySet().contains(pathToCheck.size())) {
                    solution.get(heuristicValueOfPathFromFringe).add(pathToCheck);
                }
                else {
                    ArrayList<ArrayList<Point>> arrayListWithPathToCheck = new ArrayList<>();
                    arrayListWithPathToCheck.add(pathToCheck);
                    solution.put(heuristicValueOfPathFromFringe, arrayListWithPathToCheck);
                }
            }
            else {
                    ...
                    for (Point child : children) {
                        if (!pathToCheck.contains(child)) {
                            ...
                            if (addChild) {
                                ...
                                if (solution.size() == 0) {
                                    addToFringe = true;
                                } else if (heuristicsOfPathToPotentiallyAdd <= solution.firstKey()) {
                                    addToFringe = true;
                                }
                                if (addToFringe) {
                                    if (fringe.keySet().contains(heuristicsOfPathToPotentiallyAdd)) {
                                        fringe.get(heuristicsOfPathToPotentiallyAdd).add(pathToPotentiallyAdd);
                                    } else {
...
////        return path.size() + averageManhatten;
        ...
        return (int) (path.size() + shortestManhatten * 1.2);
//        return path.size() * 5 + shortestManhatten * 3 + averageManhatten;
    }
